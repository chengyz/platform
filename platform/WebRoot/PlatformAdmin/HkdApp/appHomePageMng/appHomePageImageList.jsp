	...
	<table id="imagesTable" class="table table-bordered table-hover">
		<thead>
			<tr>
			    ...
			    <th field="link" limitTxtLen="20">跳转链接</th>
			    <th field="showStartTime" limitTxtLen="20">投放在app首页的开始时间</th>
			    <th field="showEndTime" limitTxtLen="20">投放在app首页的结束时间</th>
			    <th field="displayOrder" limitTxtLen="10">显示顺序</th>
			    <th field="belongApp" limitTxtLen="10">所属App</th>
			    <th field="remark" limitTxtLen="15">备注</th>
			    <th format="yesOrNoAdvert" limitTxtLen="10">是否为广告</th>
			    <th format="yesOrNoDisable" limitTxtLen="10">是否禁用</th>
			    <th format="showImage" limitTxtLen="10">图片展示</th>
...
<script type="text/javascript">
	$(function(){
		var url = 'hkdapp/appHomePage/imagesList';
	...
	function addOrUpdateData(iframeWin, params){
		if(iframeWin.getFromSerialize() == null){
			...
			$.ajax({
				 url: 'hkdapp/appHomePage/updateImages',
				 type: "POST",
				 cache:false,
	...
	function openImageWin(type, id) {
		...
		var url = 'PlatformAdmin/HkdApp/appHomePageMng/appHomePageImageDetail.jsp';
		var area = ['40%', '70%'];
		var btn = ['确定', '取消'];
		var row = null;
		if(id){
	...
	function uploadImageData(){
		$("#addDataForm", iframeWin.document).ajaxSubmit({
			url:'<%=path%>/hkdapp/appHomePage/uploadImages',
			type:'post',
	...
	function delImageByUUIDs(id){
		...
		layer.confirm(tip, function(index){
			$.ajax({
				 url: 'hkdapp/appHomePage/deleteImages',
				 type: "POST",
				 cache: false,
				 data: {
					 pageUUID:uuids
