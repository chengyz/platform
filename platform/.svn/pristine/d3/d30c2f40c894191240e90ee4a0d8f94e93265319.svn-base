		...
		<div class="float-right">
		    <form id="findRecord_form" method="post" class="form-inline" style="margin-top:-6px;">
			    ...
			    <div class="form-group">
					...
					<div class="field">
						<input id="startTime" class="input" type="text" name="startTime" size="25"  placeholder="请选取收钱起始时间查询" title="请选取收钱起始时间查询" />
					</div>
					<div class="field">
						<input id="endTime" class="input" type="text" name="endTime" size="25"  placeholder="请选取收钱终止时间查询" title="请选取收钱终止时间查询" />
					</div>
				</div>
	...
	<table id="recordTable" class="table table-bordered table-hover">
		<thead>
			<tr>
			    ...
			    <th field="earnMoney" limitTxtLen="15">收入金额</th>
			    <th field="earnTime" limitTxtLen="20">收钱的时间</th>
			    <th field="earnDetails" limitTxtLen="20">收入详情</th> 
...
<script type="text/javascript">
	$(function(){
		var url = 'hkdapp/vipEarnRecord/earnRecordlist';
	...
	function openRecordWin(type, id) {
		...
		var url = 'PlatformAdmin/HkdApp/cashBalanceMng/earnRecordDetail.jsp';
		var area = ['40%', '45%'];
		var row = null;
		if(id){
			row = datas[id];
