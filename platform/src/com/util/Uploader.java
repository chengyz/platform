...
public class Uploader {
		...
		try {
			...
			while (fii.hasNext()) {
				...
				if (!fis.isFormField()) {
					...
					System.out.println("11111111111111111111111"+this.getPhysicalPath(this.url));
	...
	public void upload(String fileName) throws Exception {
		...
		DiskFileItemFactory dff = new DiskFileItemFactory();
		String savePath = this.getAppFolder(this.savePath);
		dff.setRepository(new File(savePath));
		try {
			ServletFileUpload sfu = new ServletFileUpload(dff);
			sfu.setSizeMax(this.maxSize * 1024);
			...
			while (fii.hasNext()) {
				...
				if (!fis.isFormField()) {
					this.originalName = fis.getName().substring(fis.getName().lastIndexOf(System.getProperty("file.separator")) + 1);
					System.out.println("1111111111"+this.getPhysicalPath(this.url));
					if (!this.checkFileType(this.originalName)) {
						this.state = this.errorInfo.get("TYPE");
						continue;
					}
	 ...
	 * @return 
	 */
	private String getAppFolder(String path) {
		File dir = new File(this.getPhysicalPath(path));
		if (!dir.exists()) {
			try {
				dir.mkdirs();
